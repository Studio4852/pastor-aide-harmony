  ...
  <div className="max-w-7xl mx-auto space-y-6">
    ...
    <div className="grid grid-cols-4 gap-4">
      {[
        ...
        <div key={m.label} className="bg-card border border-border rounded-xl p-5 animate-fade-in">
    ...
    <div className="grid grid-cols-2 gap-4">
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={weeklySteps}>
            ...
            <XAxis dataKey="day" tick={{ fontSize: 12 }} stroke="hsl(215, 10%, 50%)" />
            <YAxis tick={{ fontSize: 12 }} stroke="hsl(215, 10%, 50%)" />
            <Tooltip />
            <Bar dataKey="steps" fill="hsl(174, 62%, 40%)" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="space-y-4">
          {[
            { label: "Steps", current: 5600, goal: 10000, color: "bg-primary" },
            { label: "Water", current: 1.8, goal: 3, unit: "L", color: "bg-info" },
            { label: "Sleep", current: 6.5, goal: 8, unit: "hrs", color: "bg-success" },
            { label: "Exercise", current: 20, goal: 30, unit: "min", color: "bg-warning" },
          ].map((g) => (
            <div key={g.label}>
